...
def generate(gen, out_path, index, fixed_font=None):
    ...
    if fixed_font:
        raise RuntimeError("Temporary blocking this")
    ...
    with open(label_path, "a") as label_file, open(info_path, "a") as info_file:
        for dat, font in tqdm(gen, total=gen.count):
            name = out_path / ('%08d.jpg' % index)
            dat[0].save(name)

            dec = decompose_hangul(dat[1])
            dec = ', '.join(dec)
...
def font_split(args, fonts, out_path):
    ...
    getgen = lambda font: KoreanTextGeneratorLegacy("file", fonts=[font], dict='ksx1001.txt', count=2350, width=args.size, size=args.size)
...
def random_split(args, fonts, out_path):
    generators = []
    for font in fonts:
        gen = KoreanTextGeneratorLegacy("file", fonts=[font], dict='ksx1001.txt', count=2350, width=args.size, size=args.size, shuffle=True)
